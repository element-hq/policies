...
Where you read 'the {{ policy_homeserver }} homeserver' or 'the Service' below, it refers to the services made available at **{{ policy_homeserver }}** which store your account and personal conversation history, provide integrations such as bots and bridges, and communicate via the open Matrix decentralised communication protocol with the public Matrix Network.
...
{{ policy_owner_name }} is the Data Controller for the Service. They can be contacted as per the details below:
{{ policy_owner_contact_details }}
...
This document explains Data Privacy to the Users of the {{ policy_homeserver }} homeserver.

### 1.3 The Customer and The User

Put simply, you're a Customer if you're paying (or otherwise compensating) New Vector Ltd to provide a hosted messaging service. If you have an account registered on {{ policy_homeserver }} that you use to send and receive messages, you're a User.
...
[](NB: This is a (probably the most) significant divergence from the matrix.org t's and c's)
...
The {{ policy_homeserver }} homeserver is a *decentralised* and, if the Customer has enabled federation, *open* service. If federation is enabled, to support communication between users on different homeservers or different messaging platforms, your username, display name and messages and files are sometimes shared with other services that are connected with the {{ policy_homeserver }} homeserver.

##### Federation

If the Customer has enabled federation, then the {{ policy_homeserver }} homeserver will share user data with the wider ecosystem over federation:
...
{{ policy_homeserver }} is a paid-for, hosted homeserver operating under the instructions of the Customer. All unencrypted messages and files submitted to the homeserver are visible to the Customer.
...
All of the Users' data for the {{ policy_homeserver }} homeserver resides within the same dedicated cluster. We use software best practices to guarantee that only people designated by Customer or a User as viewers of a User's data can access it. In other words, we segment User data via software. We do our best and are very confident we're doing a good job at it, but, like every other service that hosts User data on the same database, we cannot guarantee that it is immune to a sophisticated attack.
